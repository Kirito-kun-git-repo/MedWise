...
const AdminDashboardClient = () => {
    ...
    const { data: appoinments = [], isLoading: appoinmentsLoading } = useGetAppointments();
    console.log({ doctors, appoinments });

    //calculate stats from doctors and appointments data
    const stats = {
        totalDoctors: doctors.length,
        totalAppointments: appoinments.length,
        activeDoctors: doctors.filter(doc => doc.isActive).length,
        completedAppointments: appoinments.filter(app => app.status === "COMPLETED").length,

    }
    if (doctorsLoading || appoinmentsLoading) {
        return <LoadingUI />;
    }
